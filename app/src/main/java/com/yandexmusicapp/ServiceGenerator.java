...
public class ServiceGenerator {
    ...
    public static <S> S createService(Class<S> serviceClass) {
        client.addInterceptor(new Interceptor() {
            ...
            //
            // кроме того, метод Response.body() может возвращать либо спарсенный Gson'ом список,
            // либо raw string (если в YandexApi.java указать возвращаемое значение Call<ResponseBody> (что-то типо того)
            ...
            //
            // но нужны-то нам оба! (стринг для кэша, а список для адаптера)
            // поэтому было два выхода - либо изначально возвращать raw string, а потом уже вручную парсить его,
            ...
            // к тому же, при возвращении стринга пришлось бы писать кучу проверок на валидность, в то время как
            // конвертер все делает сам
            //
            ...
            public Response intercept(Chain chain) throws IOException {
                ...
                try {
                    ...
                    // для Тоста нужен контекст, которого нет (а для того, чтобы был, нужно быдлокодить)
                    // да и фича, собственно, такая себе (слишком низкоуровневая информация), меньше юзер знает - лучше спит
                    e.printStackTrace();
                }
                // делаем новый запрос
